...
function getDynamicAppearanceCss() {
  const s=getComputedStyle(document.documentElement);
  const names=['--pt-accent','--pt-accent-strong','--pt-accent-ink','--pt-accent-rgb','--pt-on-accent','--pt-pattern-alpha','--pt-document-pattern-alpha','--pattern-ink','--visual-pattern','--document-pattern','--visual-pattern-size','--document-pattern-size'];
...
async function createIsolatedRenderPage(html) {
  ...
  const dynamicAppearanceCss = getDynamicAppearanceCss();
...
async function renderPageWithHtml2Canvas(page, scale) {
  const { default: html2canvas } = await import(HTML2CANVAS_URL);
...
export async function downloadA4Pdf({ html, filename, title, subject }) {
  ...
  documentPdf.setSubject(subject || 'Documento A4 de servicio técnico');
  documentPdf.setAuthor('Presupuesto Técnico 2.0');
  documentPdf.setCreator('Presupuesto Técnico 2.0 - Taller');
  documentPdf.setProducer('Presupuesto Técnico 2.0 - PDF A4 directo');
